...
const typeNum = (arr) => {
  const newArray1 = arr.filter(value => typeof (value) === 'number');
  return newArray1;
};
...
const containsAnd = (arr) => {
  const newArray2 = arr.filter(value => value.includes('and'));
  return newArray2;
};
...
const oddValues = (arr) => {
  const newArray3 = arr.filter(number => number % 2 === 1);
  return newArray3;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const newArray4 = arr.filter(value => !forbiddenValues.includes(value));
  return newArray4;
};
