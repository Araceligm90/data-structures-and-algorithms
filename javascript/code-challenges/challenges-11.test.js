...
function transformToLis(obj) {
  ...
  let newArray2 = newArray.map((ele, i) => {
    return `<li>${ele.join(': ')}</li>`;
  });
  return newArray2;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArray = input.map(arr => arr.filter(num => (num % 5 === 0
  )));
  let squaredArray = filteredArray.map(arr => arr.amp(num => Math.pow(2, num)));
...
let findMaleAndFemale = (data) => {
  let filteredArray = data.filter(obj => obj.gender === 'male' || obj.gender === 'female' ? obj : false)
    .map(obj => obj.name);

  return filteredArray.join(' and ');
  // Solution code here...
};
...
let findShortest = (data) => {
  let characterHeight = data.reduce((acc, cur) => parseInt(acc.height) > parseInt(cur.height) ? acc = cur : acc, { height: '3000' });
  return characterHeight.name;
};
